"""
价格位置因子 - alpha_120cq (反转版)
版本: v2.0_reversal
更新日期: 2026-01-04

核心逻辑: 当日收盘价在120日有效交易日序列中的分位数 - 反转版本

计算公式:
    alpha_120cq_reversal = 1 - ((rank - 1) / (N - 1))
    其中:
...
class PriPos120Dv2ReversalFactor:
    """alpha_120cq反转因子计算类"""
    ...
    def calculate(self, price_df: pd.DataFrame, target_date: str) -> pd.DataFrame:
        ...
        Returns:
            DataFrame包含ts_code, alpha_120cq_reversal
        """
        if len(price_df) == 0:
            return pd.DataFrame()
        ...
        for ts_code, group in price_df.groupby('ts_code'):
            ...
            # 关键修正: 反转 (1 - alpha_120cq)
            alpha_120cq_reversal = 1 - alpha_120cq

            results.append({
                'ts_code': ts_code,
                'alpha_120cq_reversal': alpha_120cq_reversal,
            })

        df_result = pd.DataFrame(results)

        if len(df_result) > 0:
            logger.info(f"alpha_120cq反转因子计算完成: {len(df_result)}只股票")
    ...
    def calculate_by_period(
        ...
        print(f"\n{'='*80}")
        print(f"计算alpha_120cq反转因子: {target_date}")
        print(f"{'='*80}")
    ...
    def get_factor_stats(self, factor_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'alpha_120cq_reversal' in factor_df.columns:
            valid_data = factor_df['alpha_120cq_reversal'].dropna()
            stats['alpha_120cq_reversal_mean'] = valid_data.mean()
            stats['alpha_120cq_reversal_std'] = valid_data.std()
            stats['alpha_120cq_reversal_min'] = valid_data.min()
            stats['alpha_120cq_reversal_max'] = valid_data.max()
            stats['alpha_120cq_reversal_median'] = valid_data.median()

        return stats


# 工厂函数
def create_factor(version: str = 'standard') -> PriPos120Dv2ReversalFactor:
    """
    创建指定版本的alpha_120cq反转因子计算器

    Args:
        version: 因子版本 ('standard', 'conservative', 'aggressive')

    Returns:
        PriPos120Dv2ReversalFactor实例
    """
    try:
        params = get_factor_param('alpha_120cq', version)
        logger.info(f"创建alpha_120cq反转因子 - 版本: {version}, 参数: {params}")
        return PriPos120Dv2ReversalFactor(params)
