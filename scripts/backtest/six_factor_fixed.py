...
六因子策略回测脚本 - 修复版
修复内容:
1. 统一时间对齐: 确保基准收益和投资组合收益使用相同的时间周期
2. 修正数据获取逻辑: 移除不必要的缓冲期，确保数据充足
3. 优化调仓日期处理: 确保所有调仓期都被正确处理
...
from factors.valuation.alpha_peg import create_factor as create_alpha_peg
...
class SixFactorBacktestFixed:
    ...
    def __init__(self, start_date: str, end_date: str, version: str = 'standard'):
        ...
        logger.info(f"Initialize backtest: {start_date} - {end_date}")
        logger.info(f"Transaction cost: {self.total_cost:.4f} (Commission{self.commission}+Stamp{self.stamp_tax}+Slippage{self.slippage})")
    ...
    def get_nearest_trading_day(self, target_date: str) -> str:
        ...
        注意:
        - 此函数仅用于个股数据(daily_kline)
        - 指数数据(index_daily_zzsz)使用单独的get_nearest_index_day()
        - 两个数据源可能有细微差异，需要分别处理
        """
        # Check if target_date is already a trading day
        sql_check = """
        SELECT trade_date
        FROM daily_kline
        WHERE trade_date = %s
    ...
    def get_monthly_dates(self) -> List[str]:
        ...
        for dt in date_objects:
            ...
            if current_month is None:
                ...
                # Month changed, save previous month's last date
                monthly_dates.append(month_last_date.strftime('%Y%m%d'))
                current_month = month_key
                month_last_date = dt
            else:
                # Same month, update last date
                month_last_date = dt
    ...
    def get_price_data_for_period_fixed(self, stocks: List[str], target_date: str, lookback_days: int) -> pd.DataFrame:
        ...
        修复: 使用动态缓冲确保至少有lookback_days个交易日
        由于202409等月份交易日较少，需要动态调整起始日期
        """
        if not stocks:
            return pd.DataFrame()
        ...
        # 先尝试使用lookback_days作为缓冲
        initial_start_dt = target_dt - timedelta(days=lookback_days + 20)  # 从较大的缓冲开始
    ...
    def get_benchmark_return_fixed(self, current_date: str, next_rebalance_date: str) -> float:
        ...
        修复:
        1. 使用传入的日期，不调用get_nearest_trading_day
        2. 如果指数数据不存在，自动查找最近的交易日
        """
        try:
            # 直接使用传入的日期（这些日期已经在daily_kline中验证过）
            # 但需要确保index_daily_zzsz中也有对应数据

            # 检查指数数据是否存在，如果不存在则找最近的
    ...
    def calculate_factors_for_date(self, rebalance_date: str, base_stocks: List[str]) -> pd.DataFrame:
        """Calculate all factors for given date"""
    ...
    def calculate_comprehensive_score(self, factor_data: pd.DataFrame) -> pd.DataFrame:
        """Calculate comprehensive score with corrected directions"""
        weights = self.params['weights']
        directions = self.params['directions']

        # Handle alpha_peg_zscore (special case - uses zscore column)
        ...
        for factor in ['alpha_peg', 'alpha_010', 'alpha_038', 'alpha_120cq', 'cr_qfq']:
            if factor in factor_data.columns:
                ...
                if directions[factor] == 'negative':
                    # For negative direction (smaller is better), use rank in reverse
                    factor_data[f'rank_{factor}'] = factor_data[factor].rank(method='dense', ascending=True)
                else:
                    # For positive direction (larger is better)
    ...
    def run_backtest(self) -> Dict:
        """Execute backtest main process - 修复版"""
        logger.info("=" * 80)
        logger.info("Starting backtest (FIXED VERSION)")
        logger.info("=" * 80)
    ...
    def calculate_performance_metrics(self, results: Dict) -> pd.DataFrame:
        ...
        def annualize_volatility(returns, days):
            if days == 0:
                return 0
            # Convert to float list for numpy
        ...
        # IC统计
        if ic_values:
            logger.info(f"\nIC统计: 均值={np.mean(ic_values):.4f}, 标准差={np.std(ic_values):.4f}, IR={np.mean(ic_values)/np.std(ic_values):.4f}")
...
def run_backtest_comparison(start_date: str, end_date: str):
    """运行修复版回测"""
    print("="*80)
    print("六因子策略回测 - 修复版")
    print("="*80)
    ...
    # 运行回测
    backtest = SixFactorBacktestFixed(start_date, end_date, 'standard')
    ...
    if len(results) > 0:
        ...
        # 保存结果
        import os
        output_dir = f"/home/zcy/alpha006_20251223/results/backtest/six_factor_{start_date}_{end_date}_fixed_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        os.makedirs(output_dir, exist_ok=True)

        # 保存详细数据
        if results['dates']:
            # Pad IC values to match dates length
            ...
            # Turnover rates is a flat list of all group turnovers across all periods
            # We need to calculate average turnover per period
            turnover_rates = results['turnover_rates'] if results['turnover_rates'] else []
            avg_turnover = []
            if turnover_rates:
                # turnover_rates has 5 groups per period, so chunks of 5
        ...
        print(f"\n结果已保存至: {output_dir}")
        print("\n修复说明:")
        print("1. 统一了基准收益和投资组合收益的时间对齐")
        print("2. 移除了get_price_data_for_period的20天缓冲，确保数据充足")
        print("3. 修正了get_next_month_return的下月日期计算")
        print("4. 确保所有7个调仓期都被正确处理")
...
if __name__ == "__main__":
    # 测试修复版
    results, metrics = run_backtest_comparison('20240601', '20241231')
