...
文件output(提供什么): alpha_profit_employee因子的完整T+20回测结果（使用动态截面）
文件pos(系统局部地位): 回测执行层，提供截面价值因子策略回测功能（动态截面版本）

基于营业利润+职工现金价值因子的回测（动态截面优化版）：
    1. 计算因子：CSRank((营业利润 + 支付给职工现金) / 总市值, 公告日期)
    2. 动态截面：对于每个trade_date T，只使用ann_date ≤ T的股票进行排名
    3. T+20持有策略
    4. 包含交易成本（0.154%）
    5. 2025年全年回测

使用示例:
    python3 alpha_profit_employee_backtest_dynamic.py --start_date 20250101 --end_date 20251231
...
class AlphaProfitEmployeeBacktestDynamic:
    """
    Alpha Profit Employee 因子回测器（动态截面版）

    核心改进：
    - 使用动态截面：对于每个trade_date，只纳入ann_date ≤ trade_date的股票
    - 防止未来数据泄露
    - 确保回测的真实性
    """
    ...
    def __init__(self, start_date: str, end_date: str, hold_days: int = 20, output_dir: str = None):
        ...
        self.output_dir = output_dir or f"/home/zcy/alpha因子库/results/alpha_profit_employee/dynamic_backtest_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        logger.info(f"Alpha Profit Employee 回测器（动态截面版）初始化完成")
    ...
    def calculate_factor_dynamic(self, fina_df: pd.DataFrame, trade_dates: pd.Series) -> pd.DataFrame:
        """
        使用动态截面计算因子

        核心改进：
        - 对于每个trade_date，只使用ann_date ≤ trade_date的股票进行CSRank
        - 防止未来数据泄露
        """
        logger.info("正在计算alpha_profit_employee因子（动态截面）...")

        factor = AlphaProfitEmployeeFactor()

        # 使用动态截面计算
        result = factor.calculate(fina_df, trade_dates=trade_dates)
    ...
    def merge_factor_returns(self, factor_df: pd.DataFrame, returns_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # factor_df的trade_date是字符串格式，转换为datetime
        factor_df['trade_date'] = pd.to_datetime(factor_df['trade_date'], format='%Y%m%d')

        # returns_df的trade_date已经是datetime格式，无需转换
    ...
    def save_results(self, results: Dict, fina_df: pd.DataFrame, factor_df: pd.DataFrame):
        ...
        # 1. 保存因子数据
        factor_file = os.path.join(self.output_dir, f"alpha_profit_employee_factor_dynamic_{self.start_date}_{self.end_date}.csv")
        ...
        performance_file = os.path.join(self.output_dir, f"alpha_profit_employee_performance_dynamic_{self.start_date}_{self.end_date}.csv")
        strategy_returns.to_csv(performance_file, index=False)
        logger.info(f"  策略绩效: {performance_file}")

        # 3. 保存IC数据
        ic_file = os.path.join(self.output_dir, f"alpha_profit_employee_ic_dynamic_{self.start_date}_{self.end_date}.csv")
        results['ic_df'].to_csv(ic_file, index=False)
        logger.info(f"  IC数据: {ic_file}")

        # 4. 保存分组收益
        group_file = os.path.join(self.output_dir, f"alpha_profit_employee_groups_dynamic_{self.start_date}_{self.end_date}.csv")
        results['group_returns'].to_csv(group_file, index=False)
        logger.info(f"  分组收益: {group_file}")

        # 5. 保存交易记录（每日选股）
        trades_file = os.path.join(self.output_dir, f"alpha_profit_employee_trades_dynamic_{self.start_date}_{self.end_date}.csv")
    ...
    def generate_report(self, results: Dict):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("Alpha Profit Employee 因子回测报告（动态截面版）\n")
            f.write("=" * 80 + "\n\n")
            ...
            f.write("  核心改进: 动态截面 - 对于每个trade_date T，只使用ann_date ≤ T的股票\n")
            f.write("  因子类型: 截面价值因子\n")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='Alpha Profit Employee 因子回测（动态截面版）')
    ...
    try:
        # 创建回测器
        backtest = AlphaProfitEmployeeBacktestDynamic(
        ...
        # 2. 生成交易日期序列（用于动态截面）
        # 获取所有可能的交易日期
        trade_dates = pd.date_range(start=args.start_date, end=args.end_date, freq='D')
        # 转换为字符串格式
        trade_dates_str = trade_dates.strftime('%Y%m%d')

        logger.info(f"准备进行动态截面计算，交易日期数: {len(trade_dates_str)}")

        # 3. 计算因子（使用动态截面）
        factor_df = backtest.calculate_factor_dynamic(fina_df, trade_dates_str)
