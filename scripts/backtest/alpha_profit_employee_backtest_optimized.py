"""
文件input(依赖外部什么): core.utils, pandas, numpy, argparse, logging, factors.calculation.alpha_profit_employee_optimized
文件output(提供什么): alpha_profit_employee优化因子的完整T+20回测结果
文件pos(系统局部地位): 回测执行层，提供优化截面价值因子策略回测功能

基于营业利润+职工现金价值因子（取反优化）的回测：
...
使用示例:
    python3 alpha_profit_employee_backtest_optimized.py --start_date 20250101 --end_date 20251231
...
class AlphaProfitEmployeeBacktestOptimized:
    """
    Alpha Profit Employee 优化因子回测器
    ...
    def __init__(self, start_date: str, end_date: str, hold_days: int = 20, output_dir: str = None):
        ...
        self.output_dir = output_dir or f"/home/zcy/alpha因子库/results/alpha_profit_employee/optimized_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        logger.info(f"Alpha Profit Employee 优化回测器初始化完成")
    ...
    def calculate_factor(self, fina_df: pd.DataFrame) -> pd.DataFrame:
        """使用AlphaProfitEmployeeOptimizedFactor计算因子"""
        logger.info("正在计算优化的alpha_profit_employee因子...")
    ...
    def save_results(self, results: Dict, fina_df: pd.DataFrame, factor_df: pd.DataFrame):
        ...
        # 1. 保存因子数据
        factor_file = os.path.join(self.output_dir, f"alpha_profit_employee_optimized_factor_{self.start_date}_{self.end_date}.csv")
        ...
        performance_file = os.path.join(self.output_dir, f"alpha_profit_employee_optimized_performance_{self.start_date}_{self.end_date}.csv")
        strategy_returns.to_csv(performance_file, index=False)
        logger.info(f"  策略绩效: {performance_file}")

        # 3. 保存IC数据
        ic_file = os.path.join(self.output_dir, f"alpha_profit_employee_optimized_ic_{self.start_date}_{self.end_date}.csv")
        results['ic_df'].to_csv(ic_file, index=False)
        logger.info(f"  IC数据: {ic_file}")

        # 4. 保存分组收益
        group_file = os.path.join(self.output_dir, f"alpha_profit_employee_optimized_groups_{self.start_date}_{self.end_date}.csv")
        results['group_returns'].to_csv(group_file, index=False)
        logger.info(f"  分组收益: {group_file}")

        # 5. 保存交易记录（每日选股）
        trades_file = os.path.join(self.output_dir, f"alpha_profit_employee_optimized_trades_{self.start_date}_{self.end_date}.csv")
    ...
    def generate_report(self, results: Dict):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("Alpha Profit Employee 优化因子回测报告\n")
            f.write("=" * 80 + "\n\n")
            ...
            f.write("  因子类型: 截面价值因子（取反优化）\n")
            f.write("  选股逻辑: 每日选择因子值前10%的股票\n")
            f.write("  优化说明: 原始因子方向错误，高因子值对应低收益，取反后优化\n\n")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='Alpha Profit Employee 优化因子回测')
    ...
    try:
        # 创建回测器
        backtest = AlphaProfitEmployeeBacktestOptimized(
